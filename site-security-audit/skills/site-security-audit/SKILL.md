...
description: >
  Security audit for live, deployed web apps. Use when given a URL to audit,
  when someone wants to check if their Vercel, Supabase, Cloudflare Workers,
  Firebase, or Lovable app is secure, when scanning JS bundles for hardcoded
  secrets, testing API endpoints without authentication, or running
  platform-specific misconfiguration checks. NOT for local codebase review or
  auditing skill files — use check-skill-security for that.
argument-hint: <url>
context: fork
